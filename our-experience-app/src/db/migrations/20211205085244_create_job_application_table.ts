...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable('JobApplication', (table: Knex.CreateTableBuilder) => {
    ...
    table.enum('status', ['submitted', 'selected-for-interview', 'not-selected-for-interview', 'offered', 'not-offered', 'offered-declined', 'offered-accepted']).defaultTo('submitted');
